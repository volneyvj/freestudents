...
router.get("/courses-details/:id/", (req, res, next) => {
    ...
    res.render("courses/details.hbs", {data: {courseDetalhes, user_id, classesArray}})
    })
...
router.post("/signcourse/:course/:teacher/", (req, res) => {
    const { courseReqId, teacher } = req.params;
  const student = req.session.currentUser._id;
    ...
    Schedule.create({course:courseReqId, teacher, status, student, classes_completed, schedule_dates: [date1, date2, date3, date4, date5]})
